...
int main(int argc, char * argv[])
{
  ...
  factory.registerFromPlugin(loader.getOSName("convert_color_bt_node"));
  ...
  factory.registerFromPlugin(loader.getOSName("is_gesturing_bt_node"));
  std::string pkgpath = ament_index_cpp::get_package_share_directory("bt_test");
  std::string xml_file = pkgpath + "/bt_xml/find_person_test.xml";
  ...
  std::cout << "Find Person Test Finished with status: " << status << std::endl;
  rclcpp::shutdown();
  return 0;
}
