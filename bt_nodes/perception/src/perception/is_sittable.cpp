...
BT::NodeStatus IsSittable::tick()
{
  ...
  if (is_place_to_sit_ && !is_person_) {
    ...
    try {
      ...
      RCLCPP_ERROR(
        node_->get_logger(), "Could not transform %s to %s: %s", camera_frame_.c_str(), "base_link",
        ex.what());
    ...
    RCLCPP_INFO(
      node_->get_logger(), "[IsSittable] cam_frame position %f %f %f",
      map2cam_msg.transform.translation.x, map2cam_msg.transform.translation.y,
      map2cam_msg.transform.translation.z);
    RCLCPP_INFO(
      node_->get_logger(), "[IsSittable] chair center: %f %f %f",
      chair_detection_.center3d.position.x, chair_detection_.center3d.position.y,
      chair_detection_.center3d.position.z);
  ...
  if (is_person_) {
    ...
    if (std::get<0>(answer)) {
      ...
      try {
        ...
        RCLCPP_ERROR(
          node_->get_logger(), "Could not transform %s to %s: %s", camera_frame_.c_str(),
          "base_link", ex.what());
...
bool IsSittable::check_object_class(
  const std::string & obj, const std::vector<perception_system_interfaces::msg::Detection> & msg)
{
  bool ret = false;

  for (auto const & result : msg) {
    RCLCPP_DEBUG(
      node_->get_logger(), "[IsSittable] object: %s, score: %f, Z: %f", result.class_name.c_str(),
      result.score, result.center3d.position.z);
    if (
...
std::tuple<bool, tf2::Transform> IsSittable::check_free_space(
  const perception_system_interfaces::msg::Detection & person_detection,
  const perception_system_interfaces::msg::Detection & chair_detection)
{
  ...
  RCLCPP_DEBUG(
    node_->get_logger(), "[IsSittable] person bbox: %f %f", person_detection.bbox3d.x,
    person_detection.bbox3d.y);
  RCLCPP_DEBUG(
    node_->get_logger(), "[IsSittable] chair bbox: %f %f", chair_detection.bbox3d.x,
    chair_detection.bbox3d.y);
  // position and bbx are in meters, convert to cm:

  const auto r_person = cv::Rect(
    person_detection.center3d.position.x * 100 - person_detection.bbox3d.x * 100 / 2,
  ...
  const auto r_chair = cv::Rect(
    chair_detection.center3d.position.x * 100 - chair_detection.bbox3d.x * 100 / 2,
  ...
  if (intersection.area() == 0) {
    ...
    free_space_tf.setOrigin(
      tf2::Vector3(
        chair_detection.center3d.position.x, chair_detection.center3d.position.y,
...
geometry_msgs::msg::Vector3 IsSittable::retrieve_bb(
  const std::string & obj, const std::vector<perception_system_interfaces::msg::Detection> & msg)
{
...
geometry_msgs::msg::Pose IsSittable::retrieve_3d_pose(
  const std::string & obj, const std::vector<perception_system_interfaces::msg::Detection> & msg)
{
...
perception_system_interfaces::msg::Detection IsSittable::retrieve_detection(
  const std::string & obj, const std::vector<perception_system_interfaces::msg::Detection> & msg)
{
